namespace TheRace
{
    ...
    public class RaceEntry
    {
        ...
        public RaceEntry() { this.driver = new Dictionary<string, UnitDriver>(); }

        public int Counter => this.driver.Count;

        public string AddDriver(UnitDriver driver)
        {
            if (driver == null) { throw new InvalidOperationException(DriverInvalid); }
            if (this.driver.ContainsKey(driver.Name)) { throw new InvalidOperationException(string.Format(ExistingDriver, driver.Name)); }
        ...
        {
            if (this.driver.Count < MinParticipants) { throw new InvalidOperationException(string.Format(RaceInvalid, MinParticipants)); }
            double averageHorsePower = this.driver.Values.Select(x => x.Car.HorsePower).Average();
